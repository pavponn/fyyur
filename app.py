...
def venues():
    ...
    for venue in venues:
        ...
        venue_object = {"id": venue.id, "name": venue.name, "num_upcoming_shows": 0}
...
def search_venues():
    ...
    for venue in matched_venues:
        result_obj = {"id" : venue.id, "name" : venue.name, "num_upcoming_shows" : 0}
        response['data'].append(result_obj)
    return render_template('pages/search_venues.html', results=response, search_term=substr_to_search_for)
